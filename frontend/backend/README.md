...
1. Clone this repo and `cd poladstransparency_17744`
...
2. Clone this repo and `cd poladstransparency_17744`
...
8. If you want to use an alternate API backend set `POLADS_API_TOKEN` and `REACT_APP_POLADS_URL` in .env file.
   (To point the app at a locally running API backend set `REACT_APP_POLADS_URL=http://localhost:<port of API backend>` in the .env file)

### Getting Started

1. from repo root dir, run `yarn run build` to build frontend code for backend to serve.
1. Run `pipenv shell`
